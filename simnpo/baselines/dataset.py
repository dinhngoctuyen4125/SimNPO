...
class DepAPIDataset(Dataset):
    """Dataset for depAPI.json — forget/retain in a single JSON file.

    Each item contains:
        - "probing input": code context using deprecated API
        - "forget": deprecated API continuation (to unlearn)
    ...
    def _tokenize(self, text):
        ...
        return pad_or_trim_tensor(encoding, self.max_len, self.tokenizer.pad_token_id)
    ...
    def get_collate_fn(self):
        def collate_fn(batch):
            ...
            dict_retain = {
                "input_ids": batch_retain,
                "labels": batch_retain.clone(),
                "attention_mask": (batch_retain != self.tokenizer.pad_token_id).long()
            }
            return dict_forget, dict_retain
        return collate_fn
